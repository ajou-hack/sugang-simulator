...
class Profile:
    ...
    security_number = ""
    db = pd.read_excel('static/db/db.xlsx', engine='openpyxl', header=1, usecols="B, C, F, G, H, M", names=['sbjName', 'sbjCode', 'profName', 'credit', 'hours', 'time'])
    takingLessonsFrame = pd.DataFrame(columns=['sbjName', 'sbjCode', 'profName', 'credit', 'hours', 'time'])
...
async def index(req: Request):
    ...
    if datetime.datetime.now() > profile.start_date:
        html = 'index.html'
    return templates.TemplateResponse(html, {"request": req, "name": profile.name, "stdNumber": profile.std_number, "stdDept": profile.std_dept, "grade": profile.grade, "maxCredits": profile.max_credits})
...
def saveTlsnNoAply(args: SaveTlsnNoAplyArgs):
    ...
    if profile.security_number == args.securityNumber:
        ...
        # 최대 학점 제한을 넘은 경우
        elif profile.takingLessonsFrame['credit'].sum() + slot['credit'].sum() > int(profile.max_credits):
            return { "RESULT_MESG": f"최대 이수 학점을 초과하였습니다", "MESSAGE_CODE": -1}

        # 해당 과목이 이미 수강중인 과목이라면
        elif profile.takingLessonsFrame.loc[profile.takingLessonsFrame['sbjName'] == sbjName].shape[0] > 0:
...
def findTakingLessonInfo():
    ...
    return {"RESULT_CODE":"100", "loginStatus":"SUCCESS", "takingLessonInfoList":[takingLessonInfoList, []], "strTlsnScheValidChkMsg":"상세 일정은 공지사항을 참조하시기 바랍니다.", "strTlsnScheValidation":"0", "loginStatusMsg":""}
...
def deleteOpenLectureReg(args: DeleteOpenLectureRsgArgs):
    ...
    slot = profile.takingLessonsFrame.loc[profile.takingLessonsFrame['sbjCode'] == sbjCode]
...
async def getCaptchaAnswer(): return await captchaImg()
